        ...
        for iface in ifaces:
            ...
            try:
                ...
                for network in networks2:
                    ...
                    MAC = ':'.join('%02x' % b for b in network.dot11Bssid).upper()
                    signal_strength = network.lRssi
                    freq = network.ulChCenterFrequency

                    BSSI_Values.append({'mac': MAC, 'ssid': SSID, 'strength': signal_strength, 'frequency': freq})
...
if __name__ == '__main__':
    ...
    while (True):
        try:
            timestamp = time.time()
            test = get_BSSI()
            datas.append({'timestamp': timestamp, 'signals': test})
            time.sleep(0.1)
        except KeyboardInterrupt:
            corners.append(timestamp)
            print("corner:", timestamp)
            if len(corners) == 4:
                break

    import json
    with open('out.json', 'w') as outfile:
        json.dump({'corners': corners, 'data': datas}, outfile)
